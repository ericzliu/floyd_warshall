...
adjacency_list::adjacency_list(int node_num)
   : _nodes(node_num + 1, std::vector<edge*>())
{
}
...
std::unique_ptr<adjacency_list> read_graph(char* filename)
{
   ...
   iss >> node_num;
   iss >> edge_num;
   std::unique_ptr<adjacency_list> graph(new adjacency_list(node_num));
...
int adjacency_list::infinity = std::numeric_limits<int>::max();